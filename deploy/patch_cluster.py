...
def prep_i3instances(private_subnetid, node_groupid):
    ...
    cmd = "aws ec2 describe-instances --filters Name=vpc-id,Values='vpc-07f43291e26bec0a2' Name=tag:Name,Values='nvme-nodes.pocketcluster*' " \
...
def prep_h1instances(private_subnetid, node_groupid):
    ...
    cmd = "aws ec2 describe-instances --filters Name=vpc-id,Values='vpc-07f43291e26bec0a2' Name=tag:Name,Values='hdd-nodes.pocketcluster*' " \
           + "Name=instance-state-name,Values='running'  --query \"Reservations[*].Instances[*].InstanceId\""
    exitcode, out, err = get_exitcode_stdout_stderr(cmd)
    pattern = r'"([A-Za-z0-9_\./\\-]*)"'
    p = re.compile(pattern)
...
def prep_i2instances(private_subnetid, node_groupid):
    ...
    cmd = "aws ec2 describe-instances --filters Name=vpc-id,Values='vpc-07f43291e26bec0a2' Name=tag:Name,Values='ssd-nodes.pocketcluster*' " \
           + "Name=instance-state-name,Values='running'  --query \"Reservations[*].Instances[*].InstanceId\""
    exitcode, out, err = get_exitcode_stdout_stderr(cmd)
    pattern = r'"([A-Za-z0-9_\./\\-]*)"'
    p = re.compile(pattern)
...
def add_namenode_eni():
    ...
    cmd = "aws ec2 describe-subnets --filters Name=vpc-id,Values='vpc-07f43291e26bec0a2' Name=tag:Name,Values='pocket-kube-private' --query \"Subnets[*].SubnetId\""
    exitcode, out, err = get_exitcode_stdout_stderr(cmd)
    pattern = r'"([A-Za-z0-9_\./\\-]*)"'
    private_subnetid = re.search(pattern, out).group().strip('\"')

    # get security group for nodes
    cmd = "aws ec2 describe-security-groups --filters Name=vpc-id,Values='vpc-07f43291e26bec0a2' Name=group-name,Values='*node*' --query \"SecurityGroups[*].GroupId\""
    exitcode, out, err = get_exitcode_stdout_stderr(cmd)
    pattern = r'"([A-Za-z0-9_\./\\-]*)"'
    ...
    # get namenode eni id
    cmd = "aws ec2 describe-network-interfaces --filters Name=vpc-id,Values='vpc-07f43291e26bec0a2' Name=description,Values='*eni for namenode*' --query \"NetworkInterfaces[*].NetworkInterfaceId\""
    exitcode, out, err = get_exitcode_stdout_stderr(cmd)
    pattern = r'"([A-Za-z0-9_\./\\-]*)"'
    ...
    cmd = "aws ec2 describe-instances --filters Name=vpc-id,Values='vpc-07f43291e26bec0a2' Name=tag:Name,Values='metadata-nodes.pocketcluster*' " \
           + "Name=instance-state-name,Values='running'  --query \"Reservations[*].Instances[*].InstanceId\""
    exitcode, out, err = get_exitcode_stdout_stderr(cmd)
    pattern = r'"([A-Za-z0-9_\./\\-]*)"'
...
def add_lambda_security_group_ingress_rule():
    ...
    cmd = "aws ec2 describe-security-groups --filters Name=vpc-id,Values='vpc-07f43291e26bec0a2' Name=group-name,Values='*pocket-kube-relax*' --query \"SecurityGroups[*].GroupId\""
    exitcode, out, err = get_exitcode_stdout_stderr(cmd)
    pattern = r'"([A-Za-z0-9_\./\\-]*)"'
